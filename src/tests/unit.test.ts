...
describe('LLMToSQL', () => {
  ...
  it('should fail to initiate', () => {
    expect(() => new LLMToSQL({})).toThrow(
      'Either an ORM instance, ORM configs or SQL information must be provided.',
    );
    expect(() => new LLMToSQL({ sequelize: new MockedSequelize() })).toThrow(
      'Either an llmApi instance or llmApi configs must be provided.',
    );
    expect(() => new LLMToSQL({ sequelizeOptions: {}, sqlInfo: '' })).toThrow(
      'Either an ORM instance, ORM configs or SQL information must be provided.',
    );
    expect(
      () =>
        new LLMToSQL({
          sequelize: new MockedSequelize(),
          sequelizeOptions: {},
        }),
    ).toThrow(
      'Either an ORM instance, ORM configs or SQL information must be provided.',
    ...
    expect(
      () => new LLMToSQL({ sequelize: new MockedSequelize(), sqlInfo: '' }),
    ).toThrow(
      'Either an ORM instance, ORM configs or SQL information must be provided.',
    );
    expect(() => new LLMToSQL({ sequelizeOptions: {}, sqlInfo: '' })).toThrow(
    ...
    expect(() => new LLMToSQL({ llmApi: new MockedOpenAIChatApi({}) })).toThrow(
      'Either an ORM instance, ORM configs or SQL information must be provided.',
    );
  });
  ...
  it('should run the LLM model and return the SQL query', async () => {
    ...
    (llmToSQL['sequelizeORM'] as any).query = jest
      .fn()
      .mockResolvedValue(mockSequelizeResponse);
    llmToSQL['llmApi'].textCompletion = jest
      .fn()
      .mockResolvedValue(mockLLMApiResponse);
    const result = await llmToSQL.run(userQuery, true);
    expect(result).toEqual({
      sqlQuery: mockLLMApiResponse.content,
      results: mockSequelizeResponse,
    });
    expect(llmToSQL['llmApi'].textCompletion).toHaveBeenCalledWith(
      expect.any(String),
      {
        systemMessage: llmToSQL['systemPrompt'],
      },
    );
    expect((llmToSQL['sequelizeORM'] as any)?.query).toHaveBeenCalledWith(
      mockLLMApiResponse.content,
    );
  });
});
